...
class PlaylistRepository extends BaseRepository_1.BaseRepository {
    /**
     * Find playlist by ID
     */
    async findById(playlistId, customerId) {
        const sql = `
      SELECT
    ...
    `;
        return this.queryOne(sql, { playlistId, customerId });
    }
    /**
     * Find playlist with items
     */
    async findByIdWithItems(playlistId, customerId) {
        const sql = `
      SELECT
    ...
    `;
        const result = await this.queryMultipleResultSets(sql, { playlistId, customerId });
        if (result.recordsets[0].length === 0) {
            return null;
        }
        const playlist = result.recordsets[0][0];
        const itemsData = result.recordsets[1];
    ...
    async update(playlistId, customerId, data) {
        const updates = [];
        const params = { playlistId, customerId };
     ...
     */
    async updateItem(itemId, playlistId, data) {
        const updates = [];
        const params = { itemId, playlistId };
    ...
    `;
        const result = await this.insert(sql, params);
        if (!result) {
            throw new errors_1.NotFoundError('Playlist item not found');
        }
        return result;
    }
    /**
     ...
     */
    async removeItem(itemId, playlistId) {
        const sql = `
    ...
    `;
        return this.queryMany(sql, { playlistId });
    }
}
