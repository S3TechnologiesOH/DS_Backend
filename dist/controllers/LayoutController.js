...
class LayoutController {
    ...
    async getById(req, res, next) {
        try {
            ...
            const customerId = req.user.customerId;
            const layout = await this.layoutService.getByIdWithLayers(layoutId, customerId);
            res.status(200).json({
                status: 'success',
                data: layout,
            });
    ...
    async create(req, res, next) {
        try {
            ...
            const layout = await this.layoutService.create(customerId, userId, req.body);
            res.status(201).json({
                status: 'success',
                data: layout,
            });
    ...
    async update(req, res, next) {
        try {
            const layoutId = parseInt(req.params.layoutId, 10);
            const customerId = req.user.customerId;
            const layout = await this.layoutService.update(layoutId, customerId, req.body);
            res.status(200).json({
                status: 'success',
                data: layout,
            });
    ...
    async duplicate(req, res, next) {
        try {
            ...
            const userId = req.user.userId;
            const { name } = req.body;
            const duplicatedLayout = await this.layoutService.duplicate(layoutId, customerId, userId, name);
            res.status(201).json({
                status: 'success',
    ...
    async addLayer(req, res, next) {
        try {
            const layoutId = parseInt(req.params.layoutId, 10);
            const customerId = req.user.customerId;
            const layer = await this.layoutService.addLayer(layoutId, customerId, req.body);
            res.status(201).json({
                status: 'success',
                data: layer,
            });
    ...
    async updateLayer(req, res, next) {
        try {
            ...
            const customerId = req.user.customerId;
            const layer = await this.layoutService.updateLayer(layerId, layoutId, customerId, req.body);
            res.status(200).json({
                status: 'success',
                data: layer,
            });
