...
export class LayoutRepository extends BaseRepository {
   ...
   */
  async findById(layoutId: number, customerId: number): Promise<Layout | null> {
    const sql = `
      SELECT
    ...
    return this.queryOne<Layout>(sql, { layoutId, customerId });
  }

  /**
   * Find layout with all its layers
   */
  async findByIdWithLayers(
    layoutId: number,
    customerId: number
  ): Promise<Layout | null> {
    const sql = `
    ...
    const result = await this.queryMultipleResultSets<[Layout, LayoutLayer]>(sql, {
      layoutId,
      customerId,
    });

    const layouts = result.recordsets[0] as Layout[];
    const layers = result.recordsets[1] as LayoutLayer[];
    ...
    layout.layers = layers.map((layer) => ({
      ...layer,
      contentConfig: layer.contentConfig ? JSON.parse(layer.contentConfig as unknown as string) : null,
      styleConfig: layer.styleConfig ? JSON.parse(layer.styleConfig as unknown as string) : null,
      animationConfig: layer.animationConfig ? JSON.parse(layer.animationConfig as unknown as string) : null,
      scheduleConfig: layer.scheduleConfig ? JSON.parse(layer.scheduleConfig as unknown as string) : null,
    }));
  ...
  async findByCustomerId(
    ...
    return this.queryMany<Layout>(sql, params);
  }
  ...
  async update(
    ...
    const params: Record<string, unknown> = { layoutId, customerId };
   ...
   */
  async findLayersByLayoutId(layoutId: number): Promise<LayoutLayer[]> {
    const sql = `
      SELECT
        LayerId as layerId,
        LayoutId as layoutId,
    ...
    const layers = await this.queryMany<LayoutLayer>(sql, { layoutId });
  ...
  async createLayer(
    ...
    const layer = await this.insert<LayoutLayer>(sql, {
      layoutId,
  ...
  async updateLayer(
    ...
    const updates: string[] = [];
    const params: Record<string, unknown> = { layerId };
