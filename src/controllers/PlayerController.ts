...
export class PlayerController {
  ...
  async create(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!siteId || !name || !playerCode) {
        throw new ValidationError('siteId, name, and playerCode are required');
      }
   ...
   * - osVersion?: string
   */
  async heartbeat(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const playerId = parseInt(req.params.playerId, 10);
      ...
      if (!['Online', 'Offline', 'Error'].includes(status)) {
        throw new ValidationError('status must be Online, Offline, or Error');
      }
